...
public class CommentsController {
	...
	public String create (@PathVariable("topicId") long topicId, @Validated @ModelAttribute("form") CommentForm form, 
		...
		if(result.hasErrors()) {
			...
			model.addAttribute("message", messageSource.getMessage("comments.create.flash1",new String[] {}, locale));
			return "comments/new";
		}
...
// /topics/{topicId}/comments/new の呼び出しで newComment メソッドはコメント登録画面を表示します。
//URL の {topicId} はコメント登録対象の話題の持つ ID です。
//
// /topics/{topicId}/comment の呼び出しで create メソッドはコメントを登録します。
